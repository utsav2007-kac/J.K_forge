function renderCapabilityPage(title, slug) {
    ...
    for (const [path, data] of Object.entries(allCapabilities)) {
        ...
        cardsHtml += `
        <a href="capabilities.html?page=${pageSlug}" class="group bg-white dark:bg-[#050914] rounded-2xl border border-gray-200 dark:border-white/10 hover:border-secondary transition-all duration-300 hover:-translate-y-2 hover:shadow-xl overflow-hidden block">
            <div class="aspect-w-16 aspect-h-9 bg-gray-100 dark:bg-[#0c2b5e] relative overflow-hidden h-48 border-b border-gray-200 dark:border-white/10">
                ...
                <div class="absolute bottom-3 left-4 z-20">
                    <span class="px-3 py-1 rounded-full text-[10px] font-bold text-white bg-secondary/80 backdrop-blur-md uppercase tracking-wider font-mono">${data.subtitle || 'Capability'}</span>
                </div>
            </div>
            <div class="p-6">
                ...
                <p class="text-gray-600 dark:text-gray-400 text-sm font-light line-clamp-2 leading-relaxed">${cleanDesc || 'Advanced forging, machining and metallurgical capabilities.'}</p>
    ...
    if (!cardsHtml) {
        cardsHtml = `<div class="col-span-full text-center text-gray-500 py-10 font-mono">No capability data available.</div>`;
    }
        ...
        <div class="bg-[#0b1426] dark:bg-[#050914] text-white pt-32 pb-20 relative overflow-hidden transition-colors duration-300">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <span class="text-[#f97316] font-mono text-xs font-bold uppercase tracking-widest">ADVANCED MANUFACTURING</span>
                </div>
                ...
                <p class="text-gray-300 text-lg max-w-2xl font-light leading-relaxed">Integrated forging, heat treatment, 5-axis CNC machining, and metallurgical quality testing under one roof.</p>
            </div>
        </div>
