function renderCapabilityPage(title, slug) {
    const displayTitle = title && title !== 'capabilities' ? title.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()) : 'Capabilities';
    
    const allCapabilities = window.pageContent?.capabilitiesData || {};
    
    let html = '';
    let delay = 0;
    
    for (const [path, data] of Object.entries(allCapabilities)) {
        if (!data) continue;
        ...
        html += `
        <a href="capabilities.html?page=${pageSlug}" class="group bg-gray-50 dark:bg-[#050914] rounded-xl border border-gray-200 dark:border-white/10 hover-metallic transition-all duration-500 hover:-translate-y-3 hover:shadow-[0_15px_40px_rgba(49,130,206,0.2)] overflow-hidden block slide-up-card" style="animation-delay: ${delay}ms;">
            ...
            <div class="p-6 relative z-20">
                <div class="flex items-center gap-2 mb-3">
                    ...
                    <div class="text-[10px] font-bold text-secondary dark:text-accent font-mono uppercase tracking-widest">${data.subtitle || 'Forging Capability'}</div>
                </div>
                ...
                <p class="text-gray-600 dark:text-gray-400 text-sm font-light line-clamp-2">${cleanDesc || 'Advanced forging and manufacturing capabilities.'}</p>
            </div>
        </a>
        `;
        delay += 50;
    ...
    if (!html) {
        html = `<div class="col-span-full text-center text-gray-500 py-10 font-mono">No capability data available yet. Please run the scraper.</div>`;
    }
    ...
    return `
        ...
        <div class="bg-gray-50 dark:bg-[#050914] text-gray-900 dark:text-white pt-32 pb-20 relative overflow-hidden border-b border-gray-200 dark:border-white/5 transition-colors duration-300">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center reveal active">
                ...
                <p class="text-xl text-gray-600 dark:text-gray-400 max-w-2xl mx-auto font-light">From closed-die forging to advanced heat treatment and CNC machining, our integrated capabilities ensure end-to-end quality.</p>
            </div>
        </div>
